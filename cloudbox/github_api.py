...
class GitHubClient:
    ...
    def request(self, method, path, data=None):
        ...
        request = Request(API_ORIGIN + path, data=body, method=method, headers={
            "Authorization": f"Bearer {self.token}", "Accept": API_ACCEPT,
            "X-GitHub-Api-Version": API_VERSION, "User-Agent": "cloudbox-agent",
        ...
        try:
            ...
            status = error.code
            error.close()
            uncertain = is_write and (status >= HTTPStatus.INTERNAL_SERVER_ERROR or status == HTTPStatus.REQUEST_TIMEOUT)
            raise GitHubAPIError("github_http_error", "GitHub rejected the request.",
                                 status=status, uncertain=uncertain) from None
        except (URLError, TimeoutError, socket.timeout, OSError, HTTPException):
            raise GitHubAPIError("github_unavailable", "The GitHub response is unavailable.",
                                 uncertain=is_write) from None
        if len(raw) > MAX_API_RESPONSE_BYTES:
            raise GitHubAPIError("github_response_too_large", "The GitHub response exceeds the size limit.",
                                 uncertain=is_write)
        try:
            return json.loads(raw)
        except (UnicodeDecodeError, ValueError):
            raise GitHubAPIError("github_response_invalid", "The GitHub response is not valid JSON.",
                                 uncertain=is_write) from None
