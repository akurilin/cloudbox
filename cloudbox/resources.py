...
SINGLETON_ADDRESSES = {
    ...
    "bucket_setting": {
        "aws_s3_bucket_public_access_block.data", "aws_s3_bucket_ownership_controls.data",
        "aws_s3_bucket_server_side_encryption_configuration.data", "aws_s3_bucket_policy.tls",
...
SCOPE = {
    ...
    "excluded": ["Unrelated resources", "AWS account baseline", "Terminated VM history"],
}
...
def tag_values(tags):
    return tags if isinstance(tags, dict) else {tag["Key"]: tag["Value"] for tag in tags}


def check_tags(tags, config, *, owner=TERRAFORM_OWNER):
    values = tag_values(tags)
    if values.get("Project") != config["project_name"] or values.get("ManagedBy") != owner:
        raise CloudboxError("owner_mismatch", "An AWS target lacks this deployment's ownership tags.")
...
def check_identity(address, values, expected, config, *, allow_unknown=False):
    ...
    if identity is None or not isinstance(values, dict):
        raise CloudboxError("target_mismatch", "A Terraform resource has no inventory rule.", resource=address)
    if any(values.get(key) != value and not (allow_unknown and values.get(key) is None)
    ...
    for value in state_strings(values):
        for _, region, account in ARN_PATTERN.findall(value):
            if (account.isdigit() and account != config["aws_account_id"]) or (
                ...
                raise CloudboxError("state_mismatch", "A resource belongs to another account or region.")


def check_declarations(environment, manifest, names):
    # This spike supports flat HCL roots and a resource-free policy module.
    for label, directory in (("bootstrap", environment.bootstrap_root), ("main", environment.main_root)):
        if list(directory.glob("*.tf.json")):
            raise CloudboxError("inventory_coverage", "Add JSON configuration support to the resource checker first.")
        ...
        if declared != covered or set(MODULE_HEADER.findall(source)) != {"policy"}:
            raise CloudboxError("inventory_coverage", "Update the resource manifest and checker with Terraform changes.",
                                root=label, uncovered=sorted(declared - covered), stale=sorted(covered - declared))
    modules = environment.main_root / "modules"
    if list(modules.rglob("*.tf.json")):
        raise CloudboxError("inventory_coverage", "Add JSON module checks before deployment.")
    for path in modules.rglob("*.tf"):
        source = path.read_text()
        if RESOURCE_HEADER.search(source) or MODULE_HEADER.search(source):
            raise CloudboxError("inventory_coverage", "Add module resource support to the checker before deployment.")
    for resources in manifest.values():
        for address, resource in resources.items():
            kind = KIND_BY_TYPE.get(address.split(".", 1)[0])
            if kind is None or resource.get("kind") != kind or not resource.get("identity"):
                raise CloudboxError("inventory_coverage", "Add an inventory check for this resource type.", resource=address)
            if kind == "bucket_setting" and resource.get("covered_by") != "aws_s3_bucket.data":
                raise CloudboxError("inventory_coverage", "A bucket setting has no parent absence check.")
    # These service adapters each support one target; extra instances need code changes.
    all_resources = {address: resource for root in manifest.values() for address, resource in root.items()}
    ...
    for kind, supported in SINGLETON_ADDRESSES.items():
        selected = {address: item for address, item in all_resources.items() if item["kind"] == kind}
        if set(selected) != supported or any(item["identity"] != singleton_identities[kind] for item in selected.values()):
            raise CloudboxError("inventory_coverage", "Extend the service adapter before adding a target.", kind=kind)
    roles = {item["identity"].get("name") or item["identity"]["arn"].rsplit("/", 1)[1]
             for item in all_resources.values() if item["kind"] == "role"}
    policies = {item["identity"]["arn"] for item in all_resources.values() if item["kind"] == "policy"}
    for item in all_resources.values():
        if item["kind"] in {"role_policy", "role_attachment"} and item["identity"]["role"] not in roles:
            raise CloudboxError("inventory_coverage", "A role policy has no parent ownership check.")
        if item["kind"] == "role_attachment" and item["identity"]["policy_arn"] not in policies:
            raise CloudboxError("inventory_coverage", "A role attachment has no policy ownership check.")
...
def terraform_manifest(environment):
    ...
    result = environment.terraform(
        environment.bootstrap_root, "console", "-no-color",
        capture=True, input_text="jsonencode({names=module.policy.names, resources=module.policy.resource_manifest})\n",
    )
    document = json.loads(json.loads(result))
    manifest = document["resources"]
    if set(manifest) != {"bootstrap", "main"}:
        raise CloudboxError("inventory_coverage", "The resource manifest must define both Terraform roots.")
    check_declarations(environment, manifest, document["names"])
    expected = {environment.bootstrap_root: manifest["bootstrap"], environment.main_root: manifest["main"]}
...
def saved_resources(environment, directory, expected, config):
    ...
    for resource in state.get("resources", []):
        ...
        if resource.get("module") or resource.get("mode") != "managed":
            raise CloudboxError("unexpected_state", "Review the unexpected Terraform state before proceeding.")
...
def check_plan_coverage(directory, plan, environment):
    ...
    if declared != covered:
        raise CloudboxError("inventory_coverage", "The plan contains resources outside the inventory manifest.")
    for module in root.get("module_calls", {}).values():
        if module.get("module", {}).get("resources") or module.get("module", {}).get("module_calls"):
            raise CloudboxError("inventory_coverage", "Add module resource checks before applying this plan.")
    for change in plan.get("resource_changes", []):
        ...
        for field in ("before", "after"):
            ...
            if values is not None:
                check_identity(change["address"], values, targets, config, allow_unknown=field == "after")
...
def inventory(session, names, config, expected):
    ...
    image = optional_call(compute.get_microvm_image, RESOURCE_MISSING, imageIdentifier=names["image_arn"])
    ...
    if image:
        ...
        if image.get("imageArn") != names["image_arn"]:
            raise CloudboxError("image_mismatch", "The image ARN does not match this deployment.")
    ...
    targets = {address: resource for root in expected.values() for address, resource in root.items()}
    roles = {}
    for address, resource in targets.items():
        ...
        if kind == "role":
            ...
            if response:
                ...
                if role["Arn"] != f"arn:aws:iam::{config['aws_account_id']}:role/{name}":
                    raise CloudboxError("role_mismatch", "The role ARN does not match this deployment.")
                roles[name] = role
                present.add(address)
        elif kind == "policy":
            response = optional_call(iam.get_policy, IAM_MISSING, PolicyArn=fields["arn"])
            if response:
                check_tags(iam.list_policy_tags(PolicyArn=fields["arn"])["Tags"], config)
                present.add(address)
    for address, resource in targets.items():
        fields, kind = resource["identity"], resource["kind"]
        if kind == "role_policy" and fields["role"] in roles:
            if optional_call(iam.get_role_policy, IAM_MISSING,
                             RoleName=fields["role"], PolicyName=fields["name"]) is not None:
                present.add(address)
        elif kind == "role_attachment" and fields["role"] in roles:
            for page in iam.get_paginator("list_attached_role_policies").paginate(RoleName=fields["role"]):
                if any(policy["PolicyArn"] == fields["policy_arn"] for policy in page["AttachedPolicies"]):
                    present.add(address)
    ...
    for page in logs.get_paginator("describe_log_groups").paginate(logGroupNamePrefix=names["log_group_name"]):
        for group in page.get("logGroups", []):
            if group["logGroupName"] == names["log_group_name"]:
                ...
                check_tags(logs.list_tags_for_resource(resourceArn=arn).get("tags", {}), config)
...
def orphan_resources(session, names, config, targets, owned_roles):
    ...
    def belongs(name, tags):
        return name == project or name.startswith((f"{project}-", f"{project}/")) or tag_values(tags).get("Project") == project

    role_names = {item["identity"].get("name") or item["identity"]["arn"].rsplit("/", 1)[1]
                  for item in targets.values() if item["kind"] == "role"}
    policy_arns = {item["identity"]["arn"] for item in targets.values() if item["kind"] == "policy"}
    iam = session.client("iam", config=SDK_CONFIG)
    ...
    for role in owned_roles:
        inline = {item["identity"]["name"] for item in targets.values()
                  if item["kind"] == "role_policy" and item["identity"]["role"] == role}
        attached = {item["identity"]["policy_arn"] for item in targets.values()
        ...
        for page in iam.get_paginator("list_role_policies").paginate(RoleName=role):
            orphans.update(f"role/{role}/inline/{name}" for name in page["PolicyNames"] if name not in inline)
        for page in iam.get_paginator("list_attached_role_policies").paginate(RoleName=role):
            orphans.update(f"role/{role}/attached/{item['PolicyArn']}" for item in page["AttachedPolicies"]
    ...
    for page in s3.get_paginator("list_buckets").paginate():
        for bucket in page["Buckets"]:
            if bucket["Name"] == names["bucket_name"]:
                continue
            response = optional_call(s3.get_bucket_tagging, S3_TAGS_MISSING,
                                     Bucket=bucket["Name"], ExpectedBucketOwner=config["aws_account_id"])
    ...
    for page in secrets.get_paginator("list_secrets").paginate(IncludePlannedDeletion=True):
        for secret in page["SecretList"]:
            if secret["Name"] != names["secret_name"] and belongs(secret["Name"], secret.get("Tags", [])):
    ...
    while True:
        ...
        for item in page.get("items", []):
            if item["imageArn"] == names["image_arn"] or item["state"] == "DELETED":
                continue
            image = optional_call(compute.get_microvm_image, RESOURCE_MISSING, imageIdentifier=item["imageArn"])
...
def resource_context(environment):
    ...
    saved = {directory: saved_resources(environment, directory, targets, config)
...
def resource_report(environment, config, names, saved, found):
    ...
    empty_state = not any(state_remains(environment, directory) for directory in environment.roots)
    clean = not (found["present"] or found["image"] or active or found["secret"] or found["orphans"] or not empty_state)
    return {
        "ok": True, "environment": environment.name, "account_id": config["aws_account_id"],
        "region": config["aws_region"], "project": config["project_name"], "clean": clean,
        "local_state_empty": empty_state, "tracked_resources": len(tracked),
        "present_resources": sorted(found["present"]),
        "untracked_resources": sorted(found["present"] - tracked), "orphan_resources": found["orphans"],
        "image": names["image_arn"] if found["image"] else None, "active_vms": active,
        "bucket": names["bucket_name"], "objects": len(found["objects"]),
        "multipart_uploads": len(found["uploads"]),
        "secret": "pending_deletion" if found["secret"] and found["secret"].get("DeletedDate")
        else "active" if found["secret"] else "absent", "scope": SCOPE,
    }
...
def check_resources(environment, require_clean=False):
    ...
    if require_clean and not report["clean"]:
        raise CloudboxError("resources_remain", "This Cloudbox environment is not empty.", inventory=report)
    return report
