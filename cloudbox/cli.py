...
from .common import (
    CREDENTIAL_MARGIN_SECONDS, DEFAULT_TIMEOUT_SECONDS, MAX_PROMPT_CHARACTERS,
    MAX_RECORD_BYTES, MICROVM_SERVICE, SCHEMA_VERSION, SDK_CONFIG, TASK_STATUSES,
    CloudboxError, emit, error_record, get_record, load_deployment,
...
def build_parser():
    ...
    for command in commands.choices.values():
        command.add_argument("--json", action="store_true", help="JSON output is always enabled.")
    return parser
...
def input_spec(arguments):
    ...
    if arguments.spec is not None:
        ...
        try:
            ...
            raise CloudboxError("invalid_spec", "The job file is not valid JSON.") from error
        if not isinstance(values, dict):
            raise CloudboxError("invalid_spec", "The job file must contain an object.")
    else:
        if arguments.prompt is None:
            raise CloudboxError("missing_prompt", "Supply a prompt, - for stdin, or --spec.")
        values["prompt"] = sys.stdin.read(MAX_PROMPT_CHARACTERS + 1) if arguments.prompt == "-" else arguments.prompt
    ...
    validate_spec({"schema_version": SCHEMA_VERSION, "model": "default", "timeout_seconds": DEFAULT_TIMEOUT_SECONDS, **values})
    return values
...
class Runs:
    ...
    def submit(self, supplied):
        ...
        spec = validate_spec({
            ...
            "timeout_seconds": deployment.get("timeout_seconds", DEFAULT_TIMEOUT_SECONDS),
            **supplied,
        })
        image_version = deployment.get("image_version")
        if not isinstance(image_version, str) or not image_version.strip() or image_version.lower() == "latest":
            raise CloudboxError("image_not_selected", "Select an explicit image version in Terraform inputs.")
        image = self.compute.get_microvm_image_version(
            imageIdentifier=deployment["image_arn"], imageVersion=image_version,
        )
        if image.get("state") != "SUCCESSFUL" or image.get("status") != "ACTIVE":
            raise CloudboxError("image_unavailable", "The selected image version is not ready and active.")
        identity = str(uuid.uuid4())
        credentials = scoped_data_credentials(self.session, deployment, identity)
        payload = {
            "schema_version": SCHEMA_VERSION, "run_id": identity,
            "bucket_name": self.bucket,
            "data_credentials": {key: credentials[key] for key in ("AccessKeyId", "SecretAccessKey", "SessionToken")},
            "openrouter_secret_arn": deployment["openrouter_secret_arn"],
            "log_group_name": deployment["log_group_name"], "aws_region": deployment["aws_region"],
        }
        payload_text = json.dumps(payload, separators=(",", ":"))
        if len(payload_text.encode("utf-8")) > MAX_HOOK_PAYLOAD_BYTES:
            raise CloudboxError("payload_too_large", "Run credentials exceed the AWS payload limit.")
        spec.update({
            "run_id": identity, "submitted_at": timestamp(), "provider": "openrouter",
            "image_arn": deployment["image_arn"], "image_version": image_version,
            "required_output": "output/result.json",
            "resources": {"memory_mib": deployment["memory_mib"], "architecture": deployment["architecture"]},
        })
        put_record(self.s3, self.bucket, run_prefix(identity) + "spec.json", spec, exclusive=True)
        seconds_left = (credentials["Expiration"] - datetime.now(timezone.utc)).total_seconds()
        if seconds_left < spec["timeout_seconds"] + CREDENTIAL_MARGIN_SECONDS:
            raise CloudboxError("credentials_expire_early", "Run credentials expire before the deadline.", run_id=identity)
        try:
            ...
            response = self.compute.run_microvm(
                ...
                idlePolicy={"autoResumeEnabled": False, "maxIdleDurationSeconds": IDLE_SUSPEND_SECONDS, "suspendedDurationSeconds": 0},
                logging={"cloudWatch": {"logGroup": deployment["log_group_name"], "logStream": identity}},
                maximumDurationInSeconds=spec["timeout_seconds"],
                runHookPayload=payload_text, clientToken=identity,
            )
        except (BotoCoreError, ClientError) as error:
            raise CloudboxError("launch_unknown", "AWS launch response is unavailable. Inspect this run before resubmitting.", run_id=identity, task_status=UNKNOWN, compute_state=UNKNOWN) from error
        launch = {
            "schema_version": SCHEMA_VERSION, "run_id": identity,
            "microvm_id": response["microvmId"], "image_arn": deployment["image_arn"],
            "image_version": image_version, "log_group_name": deployment["log_group_name"],
            "log_stream_name": identity, "started_at": timestamp(),
        }
        saved = True
        try:
            put_record(self.s3, self.bucket, run_prefix(identity) + "launch.json", launch, exclusive=True)
        except (BotoCoreError, ClientError):
            saved = False
        return {"ok": True, "run_id": identity, "microvm_id": response["microvmId"],
                "task_status": UNKNOWN, "compute_state": response.get("state", UNKNOWN),
                "launch_record_saved": saved, "spec_uri": f"s3://{self.bucket}/{run_prefix(identity)}spec.json"}
    ...
    def status(self, identity):
        ...
        if launch and launch.get("microvm_id"):
            try:
                response = self.compute.get_microvm(microvmIdentifier=launch["microvm_id"])
        ...
        if status not in TASK_STATUSES:
            status = UNKNOWN
        return {"ok": True, "run_id": identity, "task_status": status, "compute_state": state,
                "compute_error": compute_error, "exists": bool(spec or result or launch),
                "submitted_at": spec.get("submitted_at") if spec else None,
                "result": result, "launch": launch}

    def list(self, arguments):
        if not 1 <= arguments.limit <= MAX_LIST_PAGE_SIZE:
            raise CloudboxError("invalid_limit", f"Use a limit from 1 to {MAX_LIST_PAGE_SIZE}.")
        request = {"Bucket": self.bucket, "Prefix": "runs/", "Delimiter": "/", "MaxKeys": arguments.limit}
        if arguments.cursor:
        ...
        for item in page.get("CommonPrefixes", []):
            ...
            if arguments.status is None or summary["task_status"] == arguments.status:
                runs.append(summary)
        return {"ok": True, "runs": runs, "next_cursor": page.get("NextContinuationToken")}

    def cancel(self, identity):
        before = self.status(identity)
        launch = before["launch"]
        if not launch or not launch.get("microvm_id"):
            raise CloudboxError("compute_unknown", "No saved VM ID is available for cancellation.", run_id=identity)
        ...
        if requested and after["compute_state"] == AWS_TERMINATED and after["result"] is None:
            result = {"schema_version": SCHEMA_VERSION, "run_id": identity, "status": "cancelled",
                      "reason": "operator_cancelled", "started_at": launch.get("started_at"),
                      "finished_at": timestamp(), "exit_code": None, "artifact_key": None,
                      "artifact_complete": False, "usage": {}}
            put_record(self.s3, self.bucket, run_prefix(identity) + "result.json", result, exclusive=True)
            after = self.status(identity)
        ...
        for name in DOWNLOAD_NAMES:
            ...
            with response["Body"] as body, path.open("xb") as destination:
                ...
                if len(contents) > MAX_RECORD_BYTES:
                    raise CloudboxError("artifact_too_large", "The saved file exceeds the download limit.")
                destination.write(contents)
            files.append({"key": key, "path": str(path), "bytes": len(contents)})
        return {"ok": True, "run_id": identity, "task_status": status["task_status"],
                "incomplete": status["task_status"] != "succeeded", "directory": str(target), "files": files}

    def logs(self, identity, follow):
        logs = self.session.client("logs", config=SDK_CONFIG)
        token, settled = None, 0
        while True:
            request = {"logGroupName": self.deployment["log_group_name"], "logStreamName": identity, "startFromHead": True}
            if token:
