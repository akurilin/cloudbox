...
export const FINISH_PARAMETERS = {
  ...
  properties: {
    ...
    summary: {
      type: "string",
      minLength: 1,
      pattern: SUMMARY_TEXT_PATTERN.source,
    },
    result: {
      type: "object",
...
export function validateReport(params) {
  ...
  if (Reflect.ownKeys(params).some((key) => !REPORT_FIELDS.has(key))) {
    throw new Error(
      "Use only status, summary, and optional result in the finish report.",
    );
  }
  if (
...
export default function registerFinish(pi) {
  ...
  pi.registerTool({
    ...
    label: "Finish",
    description:
      "End this run with a final status, summary, and optional result object. Call finish alone after other tools complete.",
