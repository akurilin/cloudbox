...
from github_access import (  # noqa: E402
    child_environment,
    github_environment,
    revoke_token,
)
...
TEST_IDENTITY = {
    "name": "test-agent[bot]",
    "email": "42+test-agent[bot]@users.noreply.github.com",
}
...
def access_payload():
    return {
        ...
        "github_token_expires_at": (datetime.now(UTC) + timedelta(hours=1)).isoformat(),
    }
...
class GitHubAccessTests(unittest.TestCase):
    def test_inherited_auth_and_git_trace_are_removed(self):
        with patch.dict(
            os.environ,
            {"GIT_TRACE": "1", "GH_TOKEN": "inherited", "GITHUB_TOKEN": "inherited"},
        ):
            environment = child_environment()
        self.assertFalse(
            any(key.startswith(("GIT_", "GH_", "GITHUB_")) for key in environment)
        )
    ...
    def test_git_helper_uses_runtime_token_without_saving_it(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            environment.update(
                github_environment(
                    access_spec(), access_payload(), workspace, time.monotonic() + 60
                )
            )
    ...
    def test_git_commit_uses_configured_bot_identity(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(
                [TEST_IDENTITY["name"], TEST_IDENTITY["email"]] * 2, identity
            )
            self.assertNotIn(TEST_TOKEN, (workspace / ".git" / "config").read_text())
    ...
    def test_image_archive_contains_shared_api_without_private_key(self):
        with zipfile.ZipFile(io.BytesIO(source_archive())) as archive:
            ...
            self.assertFalse(
                any(name.endswith(".pem") or "/" in name for name in archive.namelist())
            )
            self.assertEqual(
                (ROOT / "cloudbox" / "github_api.py").read_bytes(),
                archive.read("github_api.py"),
            )
    ...
    def supervise_with_agent(self, agent):
        ...
        payload = {
            **access_payload(),
            ...
            "openrouter_secret_arn": "test-secret",
            "data_credentials": {
                "AccessKeyId": "test",
