...
INSTALLATION_ID = 456
REPOSITORIES = [
    {"id": 42, "full_name": "example/first"},
    {"id": 84, "full_name": "example/second"},
]
...
class GitHubAuthTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.session = Mock()
        self.session.client.return_value.get_secret_value.return_value = {
            "SecretString": self.pem
        }
        self.response = {
            "token": TOKEN,
    ...
    def test_unexpected_scope_revokes_token(self, client, revoke):
        cases = [
            {"permissions": {**RUN_PERMISSIONS, "workflows": "write"}},
            {
                "repositories": [
                    *REPOSITORIES,
                    {"id": 126, "full_name": "example/extra"},
                ]
            },
        ]
        for change in cases:
            with self.subTest(change=change):
                ...
                client.return_value.request.side_effect = (
                    lambda method, path, data=None, response=response: (
                        {"id": APP_ID, "slug": "example-agent"}
                        if path == "/app"
                        else response
                    )
                )
...
class GitHubAPITests(unittest.TestCase):
    def test_rejects_external_paths_before_request(self):
        ...
        self.assertIsNone(
            NoRedirects().redirect_request(
                None, None, HTTPStatus.FOUND, "", {}, "https://example.com"
            )
        )
    ...
    def test_write_transport_failure_is_uncertain_and_not_retried(self):
        ...
        with self.assertRaises(GitHubAPIError) as raised:
            GitHubClient(TOKEN, opener=opener).request(
                "POST", "/repos/example/first/issues", {"body": "text"}
            )
