...
class GitHubCLITests(unittest.TestCase):
    def setUp(self):
        ...
        self.runs.deployment = {
            "default_model": "test/model",
            "image_version": "1.0",
            "image_arn": "image",
            "openrouter_secret_arn": "openrouter-secret",
            "log_group_name": "logs",
        ...
        self.runs.compute.get_microvm_image_version.return_value = {
            "state": "SUCCESSFUL",
            "status": "ACTIVE",
        }
        self.runs.compute.run_microvm.return_value = {
            "microvmId": "vm-1",
            "state": "RUNNING",
        }
        self.runs.bucket = "bucket"
        self.runs.environment = SimpleNamespace(name="test")
        self.access = GitHubAccess(
            {
                "repositories": [{"id": 42, "full_name": "example/project"}],
                "permissions": RUN_PERMISSIONS,
                "git_identity": {
                    "name": "test[bot]",
                    "email": "123+test[bot]@users.noreply.github.com",
                },
            },
            TOKEN,
        ...
        self.credentials_patch = patch(
            "cloudbox.cli.scoped_data_credentials", return_value=self.credentials
        )
        self.access_patch = patch(
            "cloudbox.cli.prepare_github_access", return_value=self.access
        )
    ...
    def test_github_token_only_enters_transient_hook(self):
        self.submit()
        payload = json.loads(
            self.runs.compute.run_microvm.call_args.kwargs["runHookPayload"]
        )
        records = [
            json.loads(call.kwargs["Body"])
            for call in self.runs.s3.put_object.call_args_list
        ]
    ...
    def test_large_payload_revokes_before_launch(self):
        self.prepare_access.return_value = GitHubAccess(
            self.access.github, "x" * MAX_HOOK_PAYLOAD_BYTES, self.access.expires_at
        )
    ...
    def test_short_token_lifetime_revokes_before_launch(self):
        ...
        self.prepare_access.return_value = GitHubAccess(
            self.access.github, TOKEN, expiry
        )
        with self.assertRaises(CloudboxError) as raised:
            self.submit()
    ...
    def test_failed_spec_save_revokes_before_launch(self):
        self.runs.s3.put_object.side_effect = ClientError(
    ...
    def test_missing_launch_response_preserves_token(self):
        self.runs.compute.run_microvm.side_effect = ReadTimeoutError(
