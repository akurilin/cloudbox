...
from cloudbox.common import MICROVM_SERVICE, SDK_CONFIG, CloudboxError, emit, error_record, operator_session
from cloudbox.environments import add_environment_argument, get_environment
from cloudbox.resources import (
    IMAGE_OWNER, RESOURCE_MISSING, S3_MISSING, VM_STOPPED, check_identity, check_plan_coverage,
    check_tags, inventory, list_vms, optional_call, resource_context, resource_report,
    saved_resources, state_remains,
)
...
def destroy_plan(environment, directory, path, expected, config):
    ...
    environment.terraform(directory, "plan", "-destroy", "-input=false", "-no-color", f"-out={path}", *refresh)
    plan = json.loads(environment.terraform(directory, "show", "-json", str(path), capture=True))
...
def main(argv=None):
    stage = "preflight"
    try:
        parser = argparse.ArgumentParser(description="Delete one Cloudbox environment, not local files.")
        ...
        parser.add_argument("--force-delete-secret", action="store_true", help="Delete the secret without recovery, for a clean rebuild.")
        arguments = parser.parse_args(argv)
        environment = get_environment(arguments.env)
        config, original, admin, names, expected, saved, found = resource_context(environment)
        report = resource_report(environment, config, names, saved, found)
        if report["untracked_resources"] or report["orphan_resources"]:
            raise CloudboxError("untracked_resources", "Restore matching state or review extra resources before teardown.", inventory=report)
        ...
        with tempfile.TemporaryDirectory(prefix="cloudbox-teardown-") as temporary:
            plans = {directory: destroy_plan(environment, directory, Path(temporary) / f"{directory.name}.tfplan", targets, config)
                     for directory, targets in expected.items()}
            emit({**report, "plan_only": True,
            ...
            if not arguments.yes:
                ...
                print(f"Type {confirmation} to approve: ", end="", file=sys.stderr, flush=True)
            ...
            guard()
            if found["image"] or found["objects"] or found["uploads"] or report["active_vms"]:
            ...
            guard()
            if plans[main_root]:
                environment.terraform(main_root, "apply", "-input=false", "-no-color", str(plans[main_root]))
            stage = "secret"
            guard()
            ...
            if plans[bootstrap_root]:
                environment.terraform(bootstrap_root, "apply", "-input=false", "-no-color", str(plans[bootstrap_root]))
        stage = "verify"
        remaining = inventory(admin, names, config, expected)
        remaining_saved = {directory: saved_resources(environment, directory, targets, config)
                           for directory, targets in expected.items()}
        report = resource_report(environment, config, names, remaining_saved, remaining)
        if report["present_resources"] or report["image"] or report["active_vms"] or report["orphan_resources"] or not report["local_state_empty"]:
            raise CloudboxError("resources_remain", "Some Cloudbox resources remain. Inspect and run teardown again.", inventory=report)
        pending = remaining["secret"]
        if pending and (arguments.force_delete_secret or not pending.get("DeletedDate")):
            raise CloudboxError("secret_remains", "The secret is not removed or scheduled for deletion.", inventory=report)
        emit({**report, "deleted": True, "all_resources_absent": report["clean"],
