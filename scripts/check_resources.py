...
def main(argv=None):
    try:
        parser = argparse.ArgumentParser(
            description="Check Terraform targets and project resource residue."
        )
        add_environment_argument(parser)
        parser.add_argument(
            "--require-clean",
            action="store_true",
            help="Fail if cloud resources or state remain.",
        )
        arguments = parser.parse_args(argv)
        emit(
            check_resources(
                get_environment(arguments.env), require_clean=arguments.require_clean
            )
        )
        return 0
    except (
