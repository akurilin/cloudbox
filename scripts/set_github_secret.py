...
def private_key_from_file(path):
    ...
    if len(raw) > MAX_PRIVATE_KEY_BYTES:
        raise CloudboxError("github_private_key_invalid", "The GitHub private key file is too large.")
...
def secret_has_value(session, secret_id):
    ...
    response = session.client("secretsmanager", config=SDK_CONFIG).describe_secret(SecretId=secret_id)
    return not response.get("DeletedDate") and any(
        CURRENT_SECRET_STAGE in stages for stages in response.get("VersionIdsToStages", {}).values()
    )
...
def store_private_key(session, deployment, secret):
    ...
    if not secret_arn:
        raise CloudboxError("github_not_configured", "Configure and apply GitHub settings before loading the key.")
    session.client("secretsmanager", config=SDK_CONFIG).put_secret_value(
        SecretId=secret_arn, SecretString=secret,
    )


def main(argv=None):
    try:
        parser = argparse.ArgumentParser(description="Store the GitHub App private key in Secrets Manager.")
        add_environment_argument(parser)
        parser.add_argument("--key-file", type=Path, required=True, help="Read the private key from this PEM file.")
