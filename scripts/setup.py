...
def check_local_state(config, environment):
    ...
    for directory in environment.roots:
        ...
        if accounts - {config["aws_account_id"]} or regions - {config["aws_region"]}:
            raise CloudboxError("state_mismatch", "The saved state conflicts with the configured account or region.")
...
def apply_stage(directory, plan_path, config, environment, *, outputs_only=False):
    ...
    plan = json.loads(environment.terraform(directory, "show", "-json", str(plan_path), capture=True))
    ...
    environment.terraform(directory, "apply", "-input=false", "-no-color", str(plan_path))
...
def wait_for_provisioner(config, environment):
    role_arn = json.loads(environment.terraform(
        environment.bootstrap_root, "output", "-json", "provisioner_role_arn", capture=True,
    ))
...
def main(argv=None):
    ...
    try:
        ...
        parser.add_argument("--env-file", type=Path, help="Read the OpenRouter key from this file instead of .env.<env>.")
        ...
        if inventory["untracked_resources"] or inventory["orphan_resources"]:
            raise CloudboxError("untracked_resources", "Resolve resources outside the selected state before setup.")
        if inventory["secret"] == "pending_deletion":
            raise CloudboxError("secret_pending_deletion", "Remove or restore the pending secret before setup.")
        ...
        with tempfile.TemporaryDirectory(prefix="cloudbox-setup-") as plan_directory:
            ...
            for stage, directory in (("bootstrap", environment.bootstrap_root), ("infrastructure", environment.main_root)):
            ...
            apply_stage(environment.main_root, plans / "image-selection.tfplan", config, environment, outputs_only=True)
        # Stop at image selection; the cloud math test is a separate operator action.
        emit({"ok": True, "ready": True, "environment": environment.name, "account_id": config["aws_account_id"],
